...
(async function ApolloServerInit() {
  ...
  const server = new ApolloServer<ProductServiceContext>({
    schema: Modules.schemas,
    csrfPrevention: true,
    plugins: [
      responseCachePlugin<ProductServiceContext>(),
  ...
  httpServer.listen(port, () => {
    console.log("\x1b[33m", `Server ready at http://localhost:${port}/product-service`, "\x1b[0m");
  });
})();
